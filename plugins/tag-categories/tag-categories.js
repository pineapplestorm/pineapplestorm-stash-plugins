...
(() => {
  ...
  async function readPluginConfig() {
    ...
    return {
      ...
      // withSubColours back-fills each category's subColours map so every
      // read path sees a colour for every sub-category, whether or not the
      // stored taxonomy predates the feature. See withSubColours.
      taxonomy: withSubColours(ours.taxonomy || DEFAULT_TAXONOMY),
  ...
  // Lookup a sub-category's own colour.
  //
  // Sub-category colours live in a per-category map keyed by sub name:
  //   { name, colour, subcategories: ["Oral", ...],
  //     subColours: { "Oral": "#55a864", ... } }
  // Keeping `subcategories` a plain string array (rather than promoting it
  // to an array of objects) means every existing consumer is untouched —
  // the reorder sort key's indexOf, the tagOrder keys, populateSubOptions,
  // validation. Only colour lookups consult the parallel map.
  //
  // Falls back to the parent category's colour when the sub has no entry,
  // so a taxonomy written before sub colours existed still renders sanely
  // even if withSubColours hasn't run on it.
  function getSubcategoryColour(taxonomy, catName, subName) {
    if (!catName || !subName) return getCategoryColour(taxonomy, catName);
    const cat = taxonomy.categories.find((c) => c.name === catName);
    if (!cat) return UNCATEGORISED_COLOUR;
    const own = cat.subColours && cat.subColours[subName];
    if (typeof own === "string" && own) return own;
    return cat.colour || DEFAULT_CATEGORY_COLOUR;
  }

  // Back-fill: stamp every sub-category that has no colour of its own with
  // a copy of its parent category's colour.
  //
  // The semantics the plugin promises are "a sub starts out matching its
  // category, then becomes independent". A new sub is stamped at creation
  // time (see addSub in the settings modal); pre-existing subs are stamped
  // here on first read. Because the stamp is a real stored value rather
  // than a lazy fallback, later changes to the CATEGORY colour do not drag
  // already-stamped subs along with them.
  //
  // Runs on every read so no consumer has to cope with a missing entry.
  // The stamped values are persisted the first time any write path
  // round-trips the taxonomy (writeAssignment, writeTaxonomy, or a save
  // from the editor), which is also the only way a category colour can be
  // changed — so by the time a user can decouple a sub, it is already
  // stamped.
  //
  // Returns a new object and never mutates the input: DEFAULT_TAXONOMY is
  // a module-level const shared by every caller.
  function withSubColours(taxonomy) {
    const cats = (taxonomy && taxonomy.categories) || [];
    return {
      ...taxonomy,
      categories: cats.map((c) => {
        const parent = c.colour || DEFAULT_CATEGORY_COLOUR;
        const subColours = { ...(c.subColours || {}) };
        for (const s of c.subcategories || []) {
          if (typeof subColours[s] !== "string" || !subColours[s]) {
            subColours[s] = parent;
          }
        }
        return { ...c, subColours };
  ...
  function buildSubCategoryRow(currentCat, currentSub, taxonomy) {
    ...
    // Each sub-category carries its own colour, so the pill/option colour
    // depends on which sub is being rendered - hence the `val` parameter.
    // Read the live cat select from the DOM each call so the colours track
    // the user switching categories without rebuilding the picker. Fall
    // back to the currentCat parameter when the lookup misses - the cat row
    // is built BEFORE it's inserted into the DOM, and buildSubCategoryRow
    // runs the first refreshPickerControl during the cat row's pre-insert
    // phase, so a naive DOM lookup returns null and we'd render the pill
    // with the uncategorised grey instead of the real colour.
    const colourFor = (val) => {
      const catEl = document.getElementById(SELECT_CAT_ID);
      const activeCat = catEl ? catEl.value || "" : currentCat;
      return getSubcategoryColour(taxonomy, activeCat, val);
    };
    ...
    try {
      ...
      catSelect.addEventListener("change", () => {
        ...
        if (subPicker) {
          ...
          const subColourFor = (val) =>
            getSubcategoryColour(taxonomy, newCat, val);
  ...
  async function injectTagDetailDisplayIfNeeded() {
    ...
    // Sub-category carries its own colour. When no subcategory is assigned
    // ("uncategorised"), use a muted colour instead.
    const subColour = subcategory
      ? getSubcategoryColour(taxonomy, category, subcategory)
  ...
  function buildCardBadge(category, subcategory, colour, subColour, catHref, subHref) {
    ...
    // category pill always shows when a category is assigned. Each pill
    // carries its own colour — the sub-category's own, which may differ
    // from its parent category's.
    wrap.appendChild(buildCardBadgePill(category, colour, catHref));
    if (subcategory) {
      wrap.appendChild(
        buildCardBadgePill(subcategory, subColour || colour, subHref)
      );
    }
  ...
  function SettingsModalBody({ initialConfig, onSaved, onClose, requestCloseRef }) {
    ...
    // Category colour. Deliberately does NOT cascade into the category's
    // sub-categories: every sub already carries its own stamped colour by
    // the time the editor opens (readPluginConfig runs withSubColours), so
    // recolouring a category leaves its subs where the user put them.
    ...
    // Sub-category colour, stored in the selected category's subColours map.
    function setSubColour(subName, newColour) {
      const next = { ...taxonomy };
      next.categories = taxonomy.categories.map((c, i) =>
        i === selectedCatIdx
          ? { ...c, subColours: { ...(c.subColours || {}), [subName]: newColour } }
          : c
      );
      setTaxonomy(next);
      markDirty();
    ...
    async function addSub() {
      ...
      next.categories = taxonomy.categories.map((c, i) => {
        ...
        return {
          ...
          // A new sub starts out matching its parent category's colour.
          // From here it is independent — recolouring the category later
          // won't move it.
          subColours: {
            ...(c.subColours || {}),
            [trimmed]: c.colour || DEFAULT_CATEGORY_COLOUR,
          },
          tagOrder,
        };
      });
    ...
    async function renameSub(subIdx) {
      ...
      next.categories = taxonomy.categories.map((c, i) => {
        ...
        // subColours is keyed by sub NAME, so a rename has to move the key
        // or the sub loses its colour and silently falls back to the
        // category's on the next read.
        const subColours = { ...(c.subColours || {}) };
        subColours[trimmed] =
          subColours[cur] || c.colour || DEFAULT_CATEGORY_COLOUR;
        delete subColours[cur];
        return { ...c, subcategories: subs, subColours, _subRenames: renames };
      });
      setTaxonomy(next);
      markDirty();
    }
    ...
    async function deleteSub(subIdx) {
      ...
      next.categories = taxonomy.categories.map((c, i) => {
        ...
        const subColours = { ...(c.subColours || {}) };
        delete subColours[cur];
        return { ...c, subcategories: subs, subColours, _subDeletes: deletions };
      });
      setTaxonomy(next);
      markDirty();
    }
    ...
    function importFromJson() {
      try {
        ...
        for (const c of parsed.categories) {
          ...
          // Sub colours are optional in pasted JSON — back-fill from the
          // category colour so an import of a pre-sub-colour taxonomy (or
          // a hand-written one) still ends up fully coloured.
          const subColours = {};
          for (const s of c.subcategories) {
            const own = c.subColours && c.subColours[s];
            subColours[s] =
              typeof own === "string" && /^#[0-9a-fA-F]{6}$/.test(own)
                ? own
                : c.colour;
          }
          c.subColours = subColours;
        }
        setTaxonomy(parsed);
        setSelectedCatIdx(0);
        markDirty();
      ...
      try {
        ...
        const clean = {
          categories: taxonomy.categories.map((c) => {
            ...
            // Emit a colour for every LIVE sub, dropping keys left behind
            // by deleted/renamed subs. Anything missing or malformed falls
            // back to the parent category's colour, which is also what a
            // brand-new sub is stamped with.
            const subColours = {};
            for (const s of c.subcategories) {
              const own = c.subColours && c.subColours[s];
              subColours[s] =
                ...
                : React.createElement(
                    ...
                    selectedCat.subcategories.map((s, subIdx) =>
                      React.createElement(
                        ...
                        // Per-sub colour swatch, mirroring the category
                        // rows in the left column. Seeded from the parent
                        // category's colour when the sub was created.
                        React.createElement("input", {
                          type: "color",
                          value:
                            (selectedCat.subColours &&
                              selectedCat.subColours[s]) ||
                            selectedCat.colour ||
                            DEFAULT_CATEGORY_COLOUR,
                          className: "tag-categories-swatch-input",
                          title: "Change sub-category colour",
                          onClick: (e) => e.stopPropagation(),
                          onChange: (e) => setSubColour(s, e.target.value),
                        }),
                        React.createElement(
                          "span",
                          { className: "tag-categories-list-name" },
  ...
  function invalidatePillColourerCache() {
    ...
    document
      ...
      .forEach((el) => {
        ...
        // v1.4.8: clear the reorder lever too so the next observer pass
        // re-evaluates chip order against the fresh taxonomy. Without
        // this, a stale --tc-order would pin chips in their old sorted
        // positions until react-select tore them down naturally.
        el.style.removeProperty("--tc-order");
      });
  ...
  function resolveTagPillColour(tagId) {
    ...
    // Sub-category colour wins when the tag has one. That's the point of
    // sub colours: in a long tag list on a scene, the finer grouping is
    // the useful one. A tag whose category has no subs, or whose sub was
    // deleted from the taxonomy, falls back to the category colour.
    let bg = cat.colour;
    const sub = entry.subcategory;
    if (sub && (cat.subcategories || []).indexOf(sub) !== -1) {
      const own = cat.subColours && cat.subColours[sub];
      if (typeof own === "string" && own) bg = own;
    }
  ...
  async function tryColourPickerPills() {
    ...
    for (const pill of pills) {
      ...
      // v1.4.8: paint via CSS variables + stylesheet rule, NOT inline.
      //
      // The previous approach set `background-color: <bg> !important` inline,
      // which works on most surfaces but loses inside Stash's tagger view.
      // The tagger's react-select wrapper passes a `styles.multiValue` prop
      // that assigns per-chip background-colors via emotion (essentially
      // `el.style.backgroundColor = "..."` assignment). That assignment
      // strips any prior `!important` flag AND replaces the value, so our
      // inline paint vanishes a frame after we set it. A CSS-rule painter
      // gated on our `data-tc-coloured` marker is immune: react-select
      // doesn't touch CSS variables on the element (it only manages the
      // standard properties its styles config returns), and a high-
      // specificity stylesheet rule with !important wins against Stash's
      // own tagger rules.
      //
      // So: set CSS variables inline (the lever) + stylesheet rule paints
      // from them (the painter). See `tag-categories.css` for the matching
      // rule. Inner label / remove elements continue to be CSS-driven via
      // the existing descendant rules.
  ...
  async function tryReorderPickerChips() {
    ...
    for (const [parent, parentChips] of byParent) {
      ...
      // Compute the expected sorted order.
      const keyed = parentChips.map((c, i) => {
        ...
        return { c, k: computeSortKeyFromTagId(chipIds[i], name) };
      });
      keyed.sort((x, y) => compareBadgeSortKeys(x.k, y.k));

      // v1.4.8: reorder via CSS `order:` not DOM `insertBefore`.
      //
      // The previous approach mutated the DOM child order, which works on
      // most surfaces but loses inside Stash's tagger view. The tagger
      // re-renders the chip block on every state update (scrape diff
      // indicators, "is-modified" highlights, etc.) and react-select
      // rebuilds chip DOM in `values` order — undoing our insertBefore a
      // frame later, every frame, forever.
      //
      // CSS `order:` on a flex child reorders visually without touching
      // DOM child order, so react-select can't undo it via re-render.
      // The chip's react-select-managed inline styles don't include
      // `order`, so writing it via a CSS variable on the inline style
      // (which the matching CSS rule in tag-categories.css reads) is
      // safe from react-select's style normalisation too.
      //
      // Already at expected order — no-op if every chip's --tc-order
      // already matches its sorted-index. Cheap attribute read per chip.
      let needSort = false;
      for (let i = 0; i < keyed.length; i++) {
        if (keyed[i].c.style.getPropertyValue("--tc-order") !== String(i)) {
          needSort = true;
          break;
        }
      }
      ...
      try {
        for (let i = 0; i < keyed.length; i++) {
          keyed[i].c.style.setProperty("--tc-order", String(i));
        }
      } catch (err) {
        console.warn(
  ...
  console.log("[tag-categories] 1.4.9 loaded");
})();
