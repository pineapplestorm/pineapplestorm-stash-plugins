...
Sub-categories carry their own colour too. A new sub-category starts out matching its parent category, then you can recolour it independently from the swatch beside its name in the editor. A tag with a sub-category takes the sub-category's colour; everything else takes its category's. On a heavily tagged scene that gives you a second level of grouping to read from: not just "this is an act", but which kind.
